...
def build_stablecoin_network_dataframe(rows: list[RwaStablecoinNetworkRow]) -> pd.DataFrame:
    """Stablecoins **Networks** tab: chain-level market-cap league (same columns as Platforms, **Network** name)."""
...
def filter_stablecoin_network_rows(
    rows: list[RwaStablecoinNetworkRow], query: str
...
def style_stablecoin_network_dataframe(df: pd.DataFrame) -> pd.io.formats.style.Styler:
    """Networks tab: green/red highlights for 7D and 30D share (same formatting as Platforms)."""
