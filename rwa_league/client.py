...
class RwaNetworkLeagueRow:
    ...
    pct_30d_raw: float  # market_share_pct_30d_change (fraction)
...
def fetch_rwa_network_league() -> tuple[list[RwaNetworkLeagueRow], str | None]:
    ...
    30D% column uses market_share_pct_30d_change (change in market share over 30 days).
    """
    ...
    for row in raw_rows:
        ...
        p30 = row.get("market_share_pct_30d_change")
        p30f = float(p30) if isinstance(p30, (int, float)) else 0.0
