...
# All pages: same markdown HTML; price_ticker.py aligns horizontal padding with .cd-ticker-shell.
SITE_NAV_CSS = """
<style>
...
def render_home_top_bar(key_suffix: str = "page", *, is_landing: bool = False) -> None:
    """
    Top strip: same HTML on every page (fixed bar + white pill) so layout matches the ticker.

    Landing uses hash anchors. Other pages use root-relative links to the main app with
    ?jd_scroll=…; streamlit_app consumes that and scrolls after load (full navigation).
    """
    if is_landing:
        home_href = "#"
        ...
        home_href = "/"
        news_href = "/?jd_scroll=news"
        market_href = "/?jd_scroll=market"

    h = escape(home_href, quote=True)
    n = escape(news_href, quote=True)
    m = escape(market_href, quote=True)
...
<div class="jd-site-nav-fixed-wrap">
  <div class="jd-site-nav-inner">
    <nav class="jd-site-nav" aria-label="Page sections">
      ...
      <a class="jd-site-link" href="{h}">Home</a>
      <a class="jd-site-link" href="{n}">News</a>
      <a class="jd-site-link" href="{m}">Market Data</a>
    </nav>
  </div>
</div>
