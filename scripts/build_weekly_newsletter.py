...
Env:
  ...
  NEWSLETTER_LIVE_KPIS — set to 0 to skip live fetches (will error as stale unless --allow-stale).
"""
...
def build_newsletter_html(
    *,
    ...
    try:
        ...
        crypto, etp, explore = apply_live_newsletter_kpis(
            crypto, etp, explore, week_end=week_end.date()
        )
    ...
    stale_banner = ""
    try:
        from newsletter_live_kpis import kpi_refresh_issues, kpi_stale_banner_html

        kpi_issues = kpi_refresh_issues(week_end.date())
        stale_banner = kpi_stale_banner_html(kpi_issues, outlook=ol)
    except Exception as exc:
        print(f"Warning: newsletter KPI freshness check skipped: {exc}", file=sys.stderr)
    section_base_kw = {
    ...
    if is_executive:
        ...
        disclaimer = (
            ...
            f"week ending {week_label} (UTC); figures from live sources as of {data_label}."
        )
    else:
        intro = (
        ...
        footer_generation_html = (
            ...
            f"live KPI fetches (CoinPaprika, CoinGecko, StockAnalysis, Farside, RWA.xyz, RSS), "
            f"with a weekly snapshot used to catch stale reprints.</p>"
        )
        disclaimer = (
...
def main() -> None:
    ...
    parser.add_argument(
        "--allow-stale",
        action="store_true",
        help="Write HTML even if live KPIs failed to refresh this week (still shows the error banner).",
    )
    ...
    if issues and not args.allow_stale:
        print(
            "Newsletter HTML was written with a KPI refresh error banner. "
            "Fix the live sources and rebuild, or pass --allow-stale for a local draft.",
            file=sys.stderr,
        )
        raise SystemExit(1)
